...
const PostPage = () => {
  ...
  useEffect(() => {
    ...
    currentPostComments.forEach(comment => {
      if(comment.reply_to && nestedComments[comment.reply_to]) {
        nestedComments[comment.reply_to].comments.push({...comment, comments:[]});
    ...
    const onlyMasterComments = Object.keys(nestedComments).filter(comment => isComment[comment]).map(commentKey => nestedComments[commentKey]);
  ...
  return (
    <Box className="mx-10">
      ...
      <hr/>
      <FormControl onKeyDown={addComment} >
        {/* <FormLabel>Email address</FormLabel> */}
        <Input type='text' value={comment} onChange={(e) => setComment(e.target.value)} required={true}/>
